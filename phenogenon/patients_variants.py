...
def get_vcf_df(**kwargs):
    ...
    ps1 = subprocess.Popen(
        ('tabix', '-h', kwargs['vcf_file'], position), stdout=subprocess.PIPE)
    ...
    bad_vs.update([
        i for i, v in gnomad_freqs.items()
        if v['gnomad_af'] is None or v['gnomad_af'] > 0.01 and v['gnomad_hom_f'] == 0.0
    ])
    ...
    bad_vs.update([i for i in gnomad_freqs if vs_count[i]
                   > 3 and gnomad_freqs[i]['pop_filter']])
