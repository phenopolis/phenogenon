...
def coverage(v, path_to_gnomad, mode='exome'):
    ...
    header = ['chrom', 'pos', 'mean', 'median',
              1, 5, 10, 15, 20, 25, 30, 50, 100, ]
    chrom, pos, ref, alt = v.split('-')
    if mode == 'exome':
        file = os.path.join(path_to_gnomad, 'coverage',
                            'exomes', 'exacv2.chr' + chrom + '.cov.txt.gz')
    elif mode == 'genome':
        file = os.path.join(path_to_gnomad, 'coverage',
                            'genomes', 'gnomad.chr' + chrom + '.cov.txt.gz')
    else:
...
def freqs(v, path_to_gnomad, mode='exome'):
    ...
    chrom, pos, ref, alt = v.split('-')
    if mode == 'exome':
        file = os.path.join(path_to_gnomad, 'vcf', 'exomes',
                            'gnomad.exomes.r2.0.1.sites.vcf.gz')
    elif mode == 'genome':
        file = os.path.join(path_to_gnomad, 'vcf', 'genomes',
                            'gnomad.genomes.r2.0.1.sites.' + chrom + '.vcf.gz')
    tb = pysam.TabixFile(file)
    ...
    for line in records:
        ...
        for ind, this_alt in enumerate(g_alts):
            v_id = helper.clean_variant(
                '-'.join([data['chrom'], data['pos'], data['ref'], this_alt]))
            if v_id == v:
                alt_ind = ind
        if alt_ind == None:
            continue
    ...
    for v in vs:
        ...
        for m in ['exome', 'genome']:
            if fs[m]:
                ...
                if 'Hemi' in fs[m]:
                    hemi_c = hemi_c + \
                        fs[m]['Hemi'] if hemi_c != None else fs[m]['Hemi']
