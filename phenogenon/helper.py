...
def check_args(compulsory_keys, kwargs, func):
    ...
    if missing:
        raise KeyError("missing keys for {}: {}".format(
            func, ', '.join(missing)))
...
def get_patient_map(**kwargs):
    ...
    for i in range(len(kwargs['cadd_steps']) - 1):
        ...
        for j in range(len(kwargs['gnomad_steps']) - 1):
            args['gr'] = (kwargs['gnomad_steps'][j],
                          kwargs['gnomad_steps'][j + 1])
...
def get_patients(**kwargs):
    ...
    not_covered_patients = set(
        s[s < len(narrow_vs) * kwargs['miss_cutoff']].index)

    if kwargs['mode'] == 'd':
        p = [k for k, v in kwargs['data']['patients'].items() if set(v)
             & narrow_vs]
    elif kwargs['mode'] == 'r':
...
def phenogenon(**kwargs):
    ...
    raw_p_h = set([kk for kk, vv in kwargs['patient_info'].items()
                   if set(hpos).issubset(set(vv['hpo']))])
    # make a np matrix
    shape1, shape2 = [], []
