...
def main(params):
    ...
    for gene in genes:
        ...
        omim = Counter(genes[gene]['omim']).most_common(1)[0][0]
        # get associated hpos
        hpos = genes[gene]['omim:hpo'][omim].keys()
        if not hpos:
            continue
        ...
        variants = helper.get_gene_variants_from_vcf(gene_ranges[gene], hpos, params)
